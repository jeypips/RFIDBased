angular.module('app-module',['form-validator','ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,bui,validate) {
	...
	function app() {
		...
		self.list = function (scope){
			...
			$http({
				method: 'POST',
				url: 'handlers/daily-records/data.php',
				data: scope.filter
		...
		self.print = function(scope) {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/daily-records/print.php',
			  data: scope.filter
			}).then(function mySucces(response) {
		...
		function print(scope,logs) {
			...
			console.log(scope.filter.pickDate);
			...
			doc.setFont('Arial');
			doc.setFontType('normal');
			doc.text(123, 34, 'Daily Attendance of '+convert(scope.filter.pickDate));
