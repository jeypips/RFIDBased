...
foreach ($_courses as $ci => $course) {
	...
	foreach ($header as $hi => $h) {
		...
		// subtotal
		$res = $con->getData("SELECT COUNT(*) as loginCountNow FROM logged_book lb INNER JOIN students s ON s.stud_id = lb.stud_id INNER JOIN course c ON c.cour_id = s.f_cour_id WHERE s.f_cour_id = '$cour' AND date(logb_login) = '$dateNow'");
		$subtotal = array(
			"total"=>$res[0]['loginCountNow']
		);
		
		// overall total
		$overall = $con->getData("SELECT COUNT(*) as overall FROM logged_book WHERE date(logb_login) = '$dateNow'");
